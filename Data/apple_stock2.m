...
rev1=csvread('/Users/jonk/test/python_diffeq/net_income.csv',1,1).*10^9;     %Apple's net income
SandP500=csvread('/Users/jonk/test/python_diffeq/GSPC.csv',1,1);             %S&P 500
cpi1=csvread('/Users/jonk/test/python_diffeq/CPI2.csv',1,1);                 %Consumer Price Index
inflation1=csvread('/Users/jonk/test/python_diffeq/inf.csv',1,1).*0.01;      %inflation rate as decimal
ir1=csvread('/Users/jonk/test/python_diffeq/EFFR.csv',1,1).*0.01;            %Fed funds rate as as decimal
gld1=csvread('/Users/jonk/test/python_diffeq/GLD.csv',1,1);                  %SPDR Gold Trust
rd1=csvread('/Users/jonk/test/python_diffeq/RandD.csv',1,1).*10^9;           %Apple's R&D Spending
pe1=csvread('/Users/jonk/test/python_diffeq/pe_apple.csv',366,1);            %P/E ratio
NASDAQ=csvread('/Users/jonk/test/python_diffeq/NASDAQ100.csv',1,1);          %NASDAQ 100 index
AAPL1=csvread('/Users/jonk/test/python_diffeq/AAPL3.csv',1,1);               %Apple's Stock Price
iphone1=csvread('/Users/jonk/test/python_diffeq/iphone_rev.csv',1,1).*10^9 ; %iphone revenue
conf1=csvread('/Users/jonk/test/python_diffeq/cci_oecd.csv',1,1);            %Consumer Confidence Index
buy1=csvread('/Users/jonk/test/python_diffeq/buyback.csv',1,1).*10^9;        %buybacks
...
y=interp1(sol.x,sol.y(8,:),time,'linear'); %linearly interpolate sol of 
                                           %stock price down to days
der=interp1(sol.x,sol.yp(8,:),time, 'linear'); %lineaerly interpolate 
                                               %derivative down to days

%Add The Randomness-->dS=muSdt+sigmaSdB                                            

%-------------------------solving for GBM----------------------------------
%{
mu=ts2func(der./y,'times',time); %drift term as function of time;
GM=gbm(mu,sigma,'StartState',AAPL(mo(v)),'StartTime',v);
dt=l/(length(y)-1);    % time increment
nperiods=length(y)-1;  % # of simulated observations
trials=500;           % # of Simulated trials 
%rng('default');
[X,T]=GM.simBySolution(nperiods,'DeltaTime',dt,'ntrials',trials);
for i=1:trials
    Brown(i,:)=X(:,i)'; %set random vector solutions as rows of Brown matrix
end
%hold on


Brown1=1/trials.*sum(Brown); %calculate average; sum the rows of Brown

%Compare stochastic DE solution to actual stock price
figure
hold on
plot(T(1:end-1),Brown1(1:end-1),'r');
plot(T(1:end-1),AAPL(mo(v):mo(v+l)-1),'b');
lgd=legend('Predicted Apple Stock Price','Actual Apple Stock Price');
lgd.FontSize = 12;        %makes legend larger
lgd.Location='northwest'; %location of legend
set(gca,'Xtick',290:295,'XTickLabel',{'8/1/18','9/1/18','10/1/18','11/1/18','12/1/18','1/1/19'}) 
%^ puts dates along the x-axis
...
t2=datetime(2019,3,1); %End Date of Data
t3=t1:t2;              %Create a vector of days between t1 and t2
plot(t3(mo(v):mo(v+l)),y,'r');         %plot DDE solution
plot(t3(mo(v):mo(v+l)),AAPL(mo(v):mo(v+l)),'b')%plots actual stock price 
%plot(sol.x,sol.y(8,:),'r');         %plot DDE solution
%plot(time,AAPL(mo(v):mo(v+l)),'b')  %plots actual stock price on same graph
lgd=legend('Predicted Apple Trend','Actual Apple Stock Price');
lgd.FontSize = 12;         %legend size
lgd.Location='northwest';  %legend location
%set(gca,'Xtick',291:295,'XTickLabel',{'9/1/18','10/1/18','11/1/18','12/1/18','1/1/19'}) 

hold off

mapes=sum(abs(A-y')./abs(A))*(100/length(A)); %mean absolute percentage error
%run appropiate for loop to calculate these (3 different loops)
%mape1(v-98)=sum(abs(A-y')./abs(A))*(100/length(A));
%mape2(v-99)=sum(abs(A-y')./abs(A))*(100/length(A)); 
%mape3(v-198)=sum(abs(A-y')./abs(A))*(100/length(A)); 
%length(find([mape1,mape2,mape3]<10)); %predictions with <10% MAPE
%mape1 <10 (32)(28-99) 44%
%mape2 <10 (69)(100-198) %69
%mape3 <10 (79) (199-293) 83%
%67.92% less than 10% MAPE
%266 total (ends on Jan 1 ,2019)
%95 since Jan 1, 2019
%--------------------------------------------------------------------------
%The following checks if any of the variables become negative
%It will output a number 1-13. For example, if 2 is printed to screen then
%x2 becomes negative. If this is a variable such as stock price that
%shouldn't be negative, investigation is needed.
%--------------------------------------------------------------------------
%{
for j=1:13
neg_sol(j,:)= sol.y(j,:)>0; %make positive elements =1, negative elements =0
non=find(neg_sol(j,:))';                 %find non-zero elemnts in each row
 if length(non)<length(sol.y(j,:))       %this means solution went negative
     B(j,:)=zeros(1,length(sol.y(j,:))); %so make whole row =0
 else B(j,:)=non;                        %otherwise row will be all 1's
 end
end

[row,col]=find(~B); %find zeros in vector, get row and column of each number
row;                %solutions that become negative because 
unique(row) 
